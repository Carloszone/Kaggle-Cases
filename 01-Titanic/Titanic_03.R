...
dat$SibSp[dat$SibSp >= 2] <- 2
dat$Parch[dat$Parch >= 2] <- 2
...
train <- cbind(Survived, dat[1:length(Survived),])[,-c(2,5,6,8)]
...
# Feature Engineering
train_x <- dat[1:length(Survived),][,-c(4,5,7)]
train_y <- train[,1] %>% factor()
test_x <- dat[-(1:length(Survived)),][,-c(4,5,7)]
...
#                   tuneGrid = grid,
#                   trControl = train_control)
#model_ada$bestTune # the best nIter is 100 and the method is "Adaboost.M1"

#### final model
grid <- expand.grid(nIter = 100, method = "Adaboost.M1")
model_ada <- train(train_x, 
                   train_y,
                   method = "adaboost",
                   tuneGrid = grid,
...
model_xgb <- train(train_x, 
                   ...
                   tuneGrid = grid,
                   trControl = train_control)
model_xgb$bestTune # best nround = 50, max_depth = 3, gamma = 0, colsample_bytree = 1, min = 1, subsample = 0.75


grid <- expand.grid(nrounds = 50,
                    max_depth = 3,
                    eta = seq(0.01,0.4,0.05),
                    gamma = 0,
                    colsample_bytree = 1,
                    min_child_weight = 1,
                    subsample = 0.75)
model_xgb <- train(train_x, 
                   ...
                   tuneGrid = grid,
                   trControl = train_control)
model_xgb$bestTune # the best eta is 0.31
...
ensemble <- function(train_x, train_y, test_x){
  ...
  model_ada <- train(train_x, 
                     ...
                     tuneGrid = grid,
                     trControl = train_control)
  pred_ada <- predict(model_ada, test_x) == 1
...
write.csv(res, "pred_09.csv", row.names = FALSE)
